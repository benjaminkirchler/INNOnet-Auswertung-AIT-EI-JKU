...
\subsection{Balance der Haushaltsausstattung}

Die Tabellen~\ref{tab:survey_equipment_balance_noe} und~\ref{tab:survey_equipment_balance_ln} vergleichen die im Rekrutierungssurvey erhobene Ausstattung und die Haushaltsmerkmale zwischen Treatment- und Kontrollgruppe. Für Linz Netz werden der dynamische und der statische Tarifarm zu einer Treatmentgruppe zusammengefasst. Der PV-Status und die PV-Leistung stammen ausschließlich aus dem Survey und nicht aus der Zählpunkt- oder Einspeiseinformation. Nicht beantwortete kategoriale Fragen werden als eigene Kategorie ausgewiesen; dadurch summieren sich die dargestellten Kategorien innerhalb jeder Gruppe auf 100\%.

\input{paper/tables/survey_equipment_balance_noe}

\input{paper/tables/survey_equipment_balance_ln}

Ergänzend wird die binäre Treatmentzuordnung innerhalb jedes Netzgebiets auf die beobachtbaren Survey-Merkmale regressiert. Die gemeinsamen Likelihood-Ratio-Tests liefern weder für Linz Netz ($p=0{,}867$) noch für Netz Oberösterreich ($p=0{,}674$) Evidenz dafür, dass die Zuordnung systematisch von den gemeinsam berücksichtigten Observablen abhängt. Die McFadden-$R^2$-Werte betragen 0,025 beziehungsweise 0,007.

\input{paper/tables/survey_assignment_logit}
...
\input{paper/tables/report_table_consumption_15m_distribution.tex}
...
Die Abbildungen~\ref{fig:report_regime1_profile} bis \ref{fig:report_regime3_profile} zeigen den Netzbezug in den drei aufeinanderfolgenden NOÖ-Signalregimen. 
...
Für die kausale Auswertung ist entscheidend, ob sich Tarif- und Kontrollgruppen vor Beginn der Intervention ähnlich entwickeln. Die Abbildungen~\ref{fig:report_pt_ln} und~\ref{fig:report_pt_noe} zeigen dazu für jede Stunde der Vorperiode die Differenz des mittleren Strombezugs zwischen Treatment- und Kontrollgruppe. Der Strombezug wird zunächst je Haushalt aus vier Viertelstundenwerten zu kWh pro Stunde summiert. Die Bänder zeigen 95\%-Konfidenzintervalle der Differenz der Gruppenmittelwerte.

Für Linz Netz werden der dynamische und der statische Tarifarm jeweils separat mit der Kontrollgruppe verglichen. Für Netz Oberösterreich wird die Tarifgruppe der Kontrollgruppe gegenübergestellt. Werte nahe null sprechen für ähnliche mittlere Verbrauchsniveaus in der jeweiligen Stunde; zeitlich systematische Abweichungen weisen auf Unterschiede zwischen den Gruppen hin. Die formalen Event-Study-Tests im Ergebnisteil greifen diese deskriptiven Muster auf.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{paper/figures/report_pt_ln_hourly_difference.png}
    \caption{Stündliche Differenz des mittleren Strombezugs in Linz Netz: dynamische beziehungsweise statische Tarifgruppe minus Kontrollgruppe. Der Stundenverbrauch wird je Haushalt aus vier Viertelstundenwerten summiert; die Bänder zeigen 95\%-Konfidenzintervalle.}
    \label{fig:report_pt_ln}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{paper/figures/report_pt_noe_hourly_difference.png}
    \caption{Stündliche Differenz des mittleren Strombezugs in Netz Oberösterreich: Tarifgruppe minus Kontrollgruppe. Der Stundenverbrauch wird je Haushalt aus vier Viertelstundenwerten summiert; das Band zeigt das 95\%-Konfidenzintervall.}
